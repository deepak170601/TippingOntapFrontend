        ...
        {data.active.length === 0 ? (
          ...
          data.active.map(event => (
            <ActiveEventCard
              key={event.id}
              event={event}
              onViewTip={() => navigation.navigate('Wallet')} // replaced by ActiveEventScreen later
              onEndEvent={async () => {
...
const styles = StyleSheet.create({
  ...
  viewHistory:  { fontSize: fontSizes.sm, color: colours.accent, fontWeight: fontWeights.semiBold },

  // Scroll
  scroll:        { flex: 1 },
  scrollContent: { padding: spacing.base },

  // Upcoming gradient card
  upcomingCard: {
    height:        140,
  ...
  sectionAction: { fontSize: fontSizes.sm, color: colours.primary, fontWeight: fontWeights.semiBold },
  ...
  // Empty
  emptyCard: {
    backgroundColor: colours.surface, borderRadius: radius.lg,
    padding: spacing.xl, alignItems: 'center',
    borderWidth: 1, borderColor: colours.border, borderStyle: 'dashed',
    marginBottom: spacing.sm,
  },
